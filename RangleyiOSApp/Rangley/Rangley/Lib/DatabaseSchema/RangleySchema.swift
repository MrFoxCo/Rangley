...
public struct RangleySchema {
    ...
    static let lookupTables = [
        ...
        """
        CREATE TABLE IF NOT EXISTS tdMeetCategory -- <-- fix
        (
    ...
    static let normalTables = [
        ...
        ///HOW DO WE Distinguish Primary Keys from tbMeetCoordinates and tbMeets
        ///they can't have the same compound primary key for this reason
        ///1. everytime the coordinates changes that genereates a new changestamp... however every time the meet
        ///changes that will not always change the meetcoordinates. for example the description of the meeting changes
        ///and nothing else
        """
        CREATE TABLE IF NOT EXISTS tbMeetCoordinates -- physical coordinates
        (   
             MeetCoordinateID          INTEGER PRIMARY KEY AUTOINCREMENT
            ...
            ,RegionRadius           REAL    NOT NULL DEFAULT 0.0 -- DO YOU EVEN NEED THIS
        );
        ...
        (
            ...
            ,MeetCoordinateID  INTEGER NOT NULL DEFAULT 0
            ,CreatedBy_UserID  INTEGER NOT NULL DEFAULT 0
            ,DttmCreatedUtc    INTEGER NOT NULL DEFAULT (strftime('%s','now'))
        );
        
        -- PK(MeetID) already exists.
        CREATE INDEX IF NOT EXISTS idx_meetids_meetcoordinateid
          ON tbMeetIDs(MeetCoordinateID);
