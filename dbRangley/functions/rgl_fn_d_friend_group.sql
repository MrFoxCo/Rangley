...
-- DELETE FRIEND GROUP
-- ============================================
CREATE OR REPLACE FUNCTION rangley.rgl_fn_d_friend_group
(
    p_cognito_sub TEXT,
...
BEGIN
    ...
    IF v_group_owner_id IS NULL THEN
        RETURN QUERY SELECT FALSE, 'Friend group not found'::TEXT;
        RETURN;
    END IF;
    
    IF v_group_owner_id != v_user_id THEN
        RETURN QUERY SELECT FALSE, 'You do not own this friend group'::TEXT;
        RETURN;
    END IF;
    ...
    -- Delete the group
    DELETE FROM rangley.tb_friend_groups
    WHERE friend_group_id = p_friend_group_id;
    
    RETURN QUERY SELECT TRUE, 'Friend group deleted successfully'::TEXT;
