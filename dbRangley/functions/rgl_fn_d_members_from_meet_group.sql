-- ============================================
-- REMOVE MEMBERS FROM MEET GROUP (BULK)
-- ============================================
CREATE OR REPLACE FUNCTION rangley.rgl_fn_d_members_from_meet_group
(
    p_cognito_sub TEXT,
...
BEGIN
    ...
    IF v_group_owner_id IS NULL THEN
        RETURN QUERY SELECT FALSE, 'Meet group not found'::TEXT, 0::INT4;
        RETURN;
    END IF;

    IF v_group_owner_id != v_user_id THEN
        RETURN QUERY SELECT FALSE, 'You do not own this meet group'::TEXT, 0::INT4;
        RETURN;
    END IF;
    ...
    RETURN QUERY SELECT TRUE,
                        format('Removed %s member(s) from group', v_removed_count)::TEXT,
                        v_removed_count;
