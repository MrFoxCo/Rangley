...
BEGIN
  ...
  JOIN rangley.vw_users            AS recipient ON recipient.user_id = ui.user_id
  -- Only keep notifications whose meet is still “up to date” (future/active)
  JOIN rangley.vw_up_to_date_meets AS utdm ON utdm.meet_id = n.meet_id
  LEFT JOIN rangley.vw_users       AS creator ON creator.user_id = n.created_by_user_id
