...
(
    ...
    accepted_count int4,
    current_user_participant_status int2
)
LANGUAGE sql
STABLE
AS $$
...
base AS (
    ...
    JOIN me ON TRUE
    WHERE
        v.meet_status_id NOT IN (2, 3, 7) -- Exclude cancelled, completed, and deleted meets
        AND (
...
SELECT
    ...
    COALESCE(b.display_name, 'Unknown User') AS display_name,

    COALESCE(ms.my_status = 7, false) AS is_owner,
