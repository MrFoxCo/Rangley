-- DROP FUNCTION rangley.rangley_fn_m_respond_to_meet_invitation(text, uuid, int2);
...
DECLARE
    ...
    v_ntype_left int2 := 11;  -- Add notification type for left if needed
    v_ntype_removed int2 := 12;  -- Add notification type for removed if needed
...
BEGIN
    ...
    -- Updated to accept: accepted, declined, left, removed
    IF p_response_status_id NOT IN (v_status_accepted, v_status_declined, v_status_left, v_status_removed) THEN
    ...
    -- Can't change status if already owner, left, or removed
    ...
    v_notification_type_id := CASE
        ...
        WHEN p_response_status_id = v_status_left THEN v_ntype_left
        WHEN p_response_status_id = v_status_removed THEN v_ntype_removed
        ELSE NULL
    END;
    
    -- Only send notification if we have a valid type and it's not the creator changing their own status
