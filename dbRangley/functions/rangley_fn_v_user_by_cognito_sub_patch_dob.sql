-- DROP FUNCTION rangley.rangley_fn_v_user_by_cognito_sub(text);

CREATE OR REPLACE FUNCTION rangley.rangley_fn_v_user_by_cognito_sub_patch_dob(p_cognito_sub text)
RETURNS table
(
	 user_uuid	     	uuid
	...
	,dob 				text
	,dttm_created_utc 	timestamp with time zone
)
LANGUAGE plpgsql
STABLE
AS $function$
