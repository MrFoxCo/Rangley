...
//    }
    ...
    // in routes.swift (near other s.* routes)
    struct SendCodeBody: Content {
        let phone_e164: String   // e.g. +13125551234
        let code: String         // already generated by you
    }

    s.post("auth", "send-verification-code") { req async throws -> HTTPStatus in
        let body = try req.content.decode(SendCodeBody.self)

        guard body.phone_e164.hasPrefix("+"),
              body.phone_e164.count >= 8 else {
            throw Abort(.badRequest, reason: "phone_e164 must be E.164 (+countrycode...)")
        }
        guard (4...10).contains(body.code.count) else {
            throw Abort(.badRequest, reason: "code must be 4–10 chars")
        }

        let sms = SMSService()
        do {
            try await sms.sendText(
                to: body.phone_e164,
                body: "Your Rangley verification code is: \(body.code)"
            )
            return .ok
        } catch {
            req.logger.error("EUM sendText failed: \(String(reflecting: error))")
            throw Abort(.failedDependency, reason: "Failed to send SMS")
        }
    }
